...
--
--   • Signed-in users: refill every balance to the full 50-credit grant and
--     restart the 24h refill window from now.
--   • Guests: clear the rolling-24h IP ledger so no guest is currently counted
--     against the 5/day limit. Only in-window rows gate uploads, so
...
UPDATE users SET credits = 50, credits_refreshed_at = datetime('now');
